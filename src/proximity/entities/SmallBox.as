package proximity.entities
{
	 ...
	 */
	public class SmallBox extends Box2DEntity implements GameEntity
	{
		...
		public function SmallBox(name:String, x:Number = 0, y:Number = 0, angle:Number = 0)
		{
			_image = new Image(new (getDefinitionByName("smallbox.png") as Class)(0, 0) as BitmapData);
			_image.centerOrigin();
		...
		override public function buildShapes(friction:Number, density:Number, restitution:Number, group:int, category:int, collmask:int):void
		{
			...
			friction = Settings.SMALL_BOX_FRICTION;
			density = Settings.SMALL_BOX_DENSITY;
			restitution = 0;
			angle = _angle;
